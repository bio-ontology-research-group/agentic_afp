...
def main(data_root, ont):
    ...
    with open(f'data/sprof.txt') as f:
        data = f.read().split('\n\n')    
        ...
        if ont == 'bp':
            ...
            ont_ind = 6
            d = 2
        sprof_terms = headers[ont_ind + d].split('; ')
    ...
    preds = []
    for i, row in enumerate(test_df.itertuples()):
        prop_annots = sprof_preds.get(row.proteins, {})
    ...
    test_df['preds'] = preds
    test_df.to_pickle(f'{data_root}/sprof_{ont}.pkl')
