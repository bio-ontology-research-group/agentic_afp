...
def main(data_root, ont, model_name, test_data_name):
    ...
    test_data_file = f'{data_root}/{model_name}_{ont}.pkl'
    ...
    eval_preds = []
    diam_df = pd.read_pickle(f'{data_root}/diamond_{ont}.pkl')
    diam_preds = diam_df['preds'].values
    for i, row in enumerate(test_df.itertuples()):
        preds = 0.5 * row.preds + 0.5 * diam_preds[i]
